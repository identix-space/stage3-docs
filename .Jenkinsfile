pipeline {
    ...
    stages {
        stage('Build docker image'){
            agent { node 'master' }
                steps {
                    gitlabCommitStatus(name: "${STAGE_NAME}",builds: [[projectId: 'freeton-id/docs', revisionHash: "${BRANCH}"],]){}
                    running("${STAGE_NAME}")
                    git([
                        url: "git@gitlab.com:freeton-id/docs.git",
                        branch: env.BRANCH,
                    ])
                    script {
                        env.registry = "repo.mind-dev.com/mind-dev"
                        env.app_name = "free-ton/docs"
        ...
        success {
             node('master'){
                 withCredentials([string(credentialsId: 'TELEGRAM_BOT_ID', variable: 'TELEGRAM_BOT_ID'),
                    ...
                    script {
                        String message = "Build #${currentBuild.number} ${SHORTCOMMIT} completed successful on free-ton-docs ${BRANCH}"
        ...
        failure {
            node('master'){
                withCredentials([string(credentialsId: 'TELEGRAM_BOT_ID', variable: 'TELEGRAM_BOT_ID'),
                                 string(credentialsId: 'TELEGRAM_BOT_TOKEN', variable: 'TELEGRAM_BOT_TOKEN')]) {
                    script {
                        String message = "Build #${currentBuild.number} ${SHORTCOMMIT} completed failure on free-ton-docs ${BRANCH}"
                        sendMessageToTelegramChanel(env.TELEGRAM_CHAT_ID, env.TELEGRAM_BOT_ID, env.TELEGRAM_BOT_TOKEN, message)
                 }
               } 
        }
